...
#################################
teams_arr = matches['Team1'].unique().copy()

def add_(teams_arr,ipl):
    
    ipl.set_index('Team1',inplace = True)
    
    for i in teams_arr:
        ipl.rename(index = {i:i.replace(" ", "_")},inplace = True)
        
    ipl.reset_index(inplace = True)
    ipl.set_index('Team2',inplace = True)
        
    for i in teams_arr:
        ipl.rename(index = {i:i.replace(" ", "_")},inplace = True)
    
    ipl.reset_index(inplace = True)
    ipl.set_index('WinningTeam',inplace = True)
...
def teamAPI(team, matches=matches):
    ...
    Teams = matches.Team1.unique().copy()
    i = np.where(Teams == team)[0][0]
    Teams = np.delete(Teams,i)
